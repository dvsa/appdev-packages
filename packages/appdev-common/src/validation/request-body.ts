 ...
 */
export function ValidateRequestBody<T>(schema: object, { errorDetails } = { errorDetails: false }) {
  return function (_target: T, _propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (body: T, res: Response, next: NextFunction) {
      ...
      if (!body) {
        return res.status(HttpStatus.BAD_REQUEST).json({ message: 'No request body detected' });
      }
