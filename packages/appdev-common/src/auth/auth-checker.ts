...
export class JWTAuthChecker {
	 ...
	 */
	execute = async ({ request }: Action, roles: string | string[] = []): Promise<boolean> => {
		// if running locally, skip the token auth and role check
		if (process.env.IS_OFFLINE === 'true' && process.env.FORCE_LOCAL_AUTH !== 'true') return true;
		...
		if (!token || token.trim()?.length === 0) {
			throw new AuthError(HttpStatus.UNAUTHORIZED, 'Missing Authorization header');
		}
		...
		if (!tokenRoles || !Array.isArray(tokenRoles) || tokenRoles.length === 0) {
			throw new AuthError(HttpStatus.UNAUTHORIZED, 'No roles found in token', 'MISSING_ROLES');
		}
		...
		if (!success) {
			throw new AuthError(HttpStatus.UNAUTHORIZED, 'Insufficient permissions', 'UNAUTHORIZED', {
