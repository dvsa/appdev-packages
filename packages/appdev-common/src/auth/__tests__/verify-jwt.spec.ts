...
describe('JwtAuthoriser', () => {
	...
	it('should use a tenant-specific JWKS endpoint when tenantId is provided', async () => {
		...
		expect(jwtVerifyMock).toHaveBeenCalledWith(
			'token',
			{
				url: new URL('https://login.microsoftonline.com/tenant-123/discovery/keys'),
			},
			expect.objectContaining({
				audience: ['client-id'],
				issuer: ['https://sts.windows.net/tenant-123/', 'https://login.microsoftonline.com/tenant-123/v2.0'],
			})
		);
	});
